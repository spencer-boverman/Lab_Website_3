...
app.get('/team_stats', function(req, res) {
	...
	var won_games = 'select COUNT(*) from football_games WHERE home_score > visitor_score;';
	var lossed_games = 'select COUNT(*) from football_games WHERE home_score < visitor_score;';
	db.task('get-everything', task => {
	    return task.batch([
	        task.any(all_games),
	        task.any(won_games),
	        task.any(lossed_games)
	    ]);
	})
	.then(data => {
...
app.post('/player_info/post', function(req, res) {
	var player_id = req.selected;
	var desired_player = 'SELECT * FROM football_players WHERE ' + player_id +' == id;';
	var games_played = 'SELECT COUNT(*) FROM football_games WHERE players @> ARRAY[' + player_id + ']::int[]'
	var query = 'SELECT name, id FROM football_players;';

	db.task('get-everything', task => {
	    return task.batch([
	        task.any(player_id),
	        task.any(desired_player),
